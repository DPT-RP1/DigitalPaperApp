...
public enum Command {
    ...
    RAW_GET("get", Collections.emptyList(), Collections.singletonList("url"), "Sends and displays a GET request to the Digital Paper"),
    ...
    SET_CONFIGURATION("set-configuration", Collections.emptyList(), Collections.singletonList("path"), "Sends the system configuration from a local file at <path>"),
    ROOT("root", Collections.singletonList(CommandOption.DRYRUN), Collections.emptyList(), "Roots the device"),
    DIAG_FETCH("diag fetch", Collections.emptyList(), Arrays.asList("remote-path", "local-path"), "Downloads a file from the diagnostic mode, after root. See doc/diagnosis_mod_map.md"),
    ...
    ADB_FETCH_EXTENSION("adb fetch-extension", Collections.emptyList(), Arrays.asList("name", "local-path"), "Retrieves extension descriptors as returned by dpt adb list-extensions"),
    ...
    ADB_REMOVE_EXTENSION("adb remove-extension", Collections.emptyList(), Collections.singletonList("name"), "Removes a user-space extension, do not use to delete offical apps."),
    ADB_INSTALL_APK("adb install-apk", Collections.emptyList(), Collections.singletonList("local-path"), "Installs an APK and enables it in the launcher"),
