...
static int init(char *argv[]) {
  ...
  struct group *group;
  cp_string filename;
  cp_string configfile;
  int grpstat;
  const char *uri=cupsBackendDeviceURI(argv);

  if ((uri != NULL) && (strncmp(uri, "cups-pdf-dpt:/", 14) == 0) && strlen(uri) > 14) {
    uri = uri + 14;
    sprintf(configfile, "%s/cups-pdf-%s.conf", CP_CONFIG_PATH, uri);
  }
  else {
    sprintf(configfile, "%s/cups-pdf-dpt.conf", CP_CONFIG_PATH);
  }
  ...
  log_event(CPDEBUG, "Opening config: ");
  log_event(CPDEBUG, "Opening config: %s", configfile);
  dump_configuration();
...
int main(int argc, char *argv[]) {
  ...
  if (argc<6 || argc>7) {
    (void) fputs("Usage: cups-pdf-dpt job-id user title copies options [file]\n", stderr);
    return 0;
  }
